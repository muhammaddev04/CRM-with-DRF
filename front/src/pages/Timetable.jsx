...
import { WEEKDAYS, weekdayLabel } from '../lib/constants'
...
function TimetableForm({ defaultValues, groups, onSubmit, submitLabel }) {
  ...
  const missing = groups.length === 0

  return (
    <form onSubmit={handleSubmit((values) => onSubmit({ ...values, group_id: Number(values.group_id) }))} className="space-y-4">
      {missing && (
        <RequirementNotice>
          You need at least one group before you can schedule a session. Add one on the <strong>Groups</strong> page first.
        </RequirementNotice>
      )}
      ...
      <div className="flex justify-end gap-3 pt-2">
        <Button type="submit" loading={isSubmitting} disabled={missing}>
          {submitLabel}
        </Button>
      </div>
    </form>
