...
function EnrollmentForm({ defaultValues, students, groups, onSubmit, submitLabel }) {
  ...
  const missing = [students.length === 0 && 'a student', groups.length === 0 && 'a group'].filter(Boolean)
  ...
  return (
    <form onSubmit={handleSubmit((values) => onSubmit({ ...values, student_id: Number(values.student_id), group_id: Number(values.group_id) }))} className="space-y-4">
      {missing.length > 0 && (
        <RequirementNotice>
          You need at least {missing.join(' and ')} before you can create an enrollment. Add {missing.length > 1 ? 'them' : 'one'} on the{' '}
          {students.length === 0 && <strong>Students</strong>}
          {students.length === 0 && groups.length === 0 && ' / '}
          {groups.length === 0 && <strong>Groups</strong>} page first.
        </RequirementNotice>
      )}
