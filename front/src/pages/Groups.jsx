...
function GroupForm({ defaultValues, courses, mentors, isEdit, onSubmit, submitLabel }) {
  const {
    register,
    handleSubmit,
  ...
  const missing = [courses.length === 0 && 'a course', mentors.length === 0 && 'a mentor'].filter(Boolean)

  return (
    <form
      onSubmit={handleSubmit(({ mentor, ...values }) =>
        onSubmit({
          ...values,
          course_id: Number(values.course_id),
          mentor_id: Number(mentor),
    ...
    >
      {missing.length > 0 && (
        <RequirementNotice>
          You need at least {missing.join(' and ')} before you can create a group. Add {missing.length > 1 ? 'them' : 'one'} on the{' '}
          {courses.length === 0 && <strong>Courses</strong>}
          {courses.length === 0 && mentors.length === 0 && ' / '}
          {mentors.length === 0 && <strong>Mentors</strong>} page first.
        </RequirementNotice>
      )}
      ...
      <Select
        label="Mentor"
        required
        hint={isEdit ? "The backend doesn't return the current mentor on this group — pick one to set/replace it." : undefined}
